...
useEffect(() => {
  ...
  const savePassword = async () => {
    if (form.site.length > 3 && form.username.length > 3 && form.password.length > 3) {
           // Generate a single UUID for this password
      const newId = uuidv4();
      
      // If editing an existing password, delete the old one first
      if (form.id) {
     ...
     closeOnClick={false} rtl={false} pauseOnFocusLoss draggable pauseOnHover theme="light" />
      ...
      <div className="p-2 pt-3 md:mycontainer flex-grow">
        ...
        <div className="text-black flex flex-col p-4 gap-6 items-center max-w-6xl mx-auto">
          <div className="w-full">
            <input value={form.site} onChange={handleChange} className="rounded-xl border-2
             border-blue-400 w-full p-4 py-3 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200 transition-all duration-300 shadow-sm" placeholder="Enter website URL" type="text" name="site" id="site"/>
          </div>

          <div className="flex flex-col md:flex-row w-full gap-4">
           <div className="md:w-3/4 ">
             <input value={form.username} onChange={handleChange} className="rounded-xl border-2
              border-blue-400 w-full p-4 py-3 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200 transition-all duration-300 shadow-sm" placeholder="Enter Username" type="text" name="username" id="username"/>
           </div>

           <div className="md:w-1/4 relative">
             <input ref={passwordRef} value={form.password} onChange={handleChange} className="rounded-xl border-2
              border-blue-400 w-full p-4 py-3 pr-12 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-200 transition-all duration-300 shadow-sm" placeholder="Enter Password" type="password" name="password" id="password"/>
        ...
        <div className="passwords max-w-7xl mx-auto">
          ...
          {passwordArray.length === 0 && <div className="text-center text-gray-600 py-8 bg-white rounded-xl shadow-sm border border-gray-200">🔒 No passwords stored yet. Start by adding your first one above!</div> }
              ...
              <tbody className="bg-gray-50">
                {passwordArray.map((item,index)=>{
                  ...
                  <td className="py-4 px-3 border-b border-gray-200 text-center">
                    <div className="flex justify-center items-center gap-2"><a href={item.site} target="blank" className="hover:text-blue-600 transition-colors font-medium">
                  {item.site}</a>
                  <div className="size-8 px-1 py-1 rounded-full hover:bg-blue-100 transition-all duration-200" onClick={()=>copyText(item.site)}><img className="cursor-pointer transition-transform duration-200
                  ...
                  <div className="size-8 px-1 py-1 rounded-full hover:bg-blue-100 transition-all duration-200" onClick={()=>copyText(item.username)}><img className="cursor-pointer transition-transform duration-200
                   hover:scale-125 hover:opacity-80" width={30} src="icons/copy.png" /></div>
                  </div>
                  </td>
                  ...
                  {/* PASSWORD */}
                  <td className="py-4 px-3 border-b border-gray-200 text-center">
                  <div className="flex justify-center items-center gap-2"><span className="break-all font-mono bg-gray-100 px-2 py-1 rounded">{item.password}</span>
                  <div className="size-8 px-1 py-1 rounded-full hover:bg-blue-100 transition-all duration-200" onClick={()=>copyText(item.password)}><img className="cursor-pointer transition-transform duration-200
                   hover:scale-125 hover:opacity-80" width={30} src="icons/copy.png" /></div>
                  </div>
                  </td>
                  ...
                  <div className="flex justify-center items-center gap-3">  
                    {/* Edit */}
                    <span onClick={()=>{editPassword(item.id)}} className="size-8 px-1 py-1 rounded-full hover:bg-green-100 transition-all duration-200"><img className="cursor-pointer transition-transform duration-200
                    hover:scale-150 hover:opacity-80" width={30} src="icons/edit.png" /></span>    
                    {/*DELETE */}
                    <span onClick={()=>{deletePassword(item.id)}} className="size-8 px-1 py-1 rounded-full hover:bg-red-100 transition-all duration-200"><img className="cursor-pointer transition-transform duration-200
